...
object MovieLensALS {
  ...
  def main(args: Array[String]) {
    ...
    val parser = new OptionParser[Params]("MovieLensALS") {
      ...
      arg[String]("<output>")
          .required()
          .text("output paths to a MovieLens dataset of ratings")
...
//	val rawdata = sc.textFile(params.input).filter(x=>{x.length<500 && x.split(",").length <= 4})

	val rawdata = sc.textFile(params.input,200)
//
//	  val num = rawdata.count()
//	  val num2 = rawdata2.count()
//	println("asdasdasd:"+num+"__"+num2)
    ...
    //val numUsers = ratings.map(_.user).distinct(100).count()
    //val numMovies = ratings.map(_.product).distinct(100).count()
   ...
   //val rmse = computeRmse(model, test, params.implicitPrefs)
    ...
    //println(s"Test RMSE = $rmse.")
      println("The model building finished.")
